...
class Out extends Component {
  constructor() {
    ...
    this.getConfig((d) => {
      if (d.data.length > 0) {
        self.setState({text: d.data[0]['numeroAlternativo1']
        });
      }
    });
  }
  ...
  submit(event) {
    ...
    this.getConfig((d) => {
      if (d.data.length > 0) {
        this.configService.patch(d.data[0]['_id'], {numeroAlternativo1: self.state.text})
      } else {
        this.configService.create({numeroAlternativo1: self.state.text});
      }
    });
  }
